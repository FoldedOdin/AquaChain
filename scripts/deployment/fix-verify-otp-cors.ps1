# Fix CORS for /api/auth/verify-otp endpoint
...
Write-Host "Fixing CORS for /api/auth/verify-otp endpoint..." -ForegroundColor Cyan
...
# Find the verify-otp resource
$verifyOtpResource = $resources.items | Where-Object { $_.path -eq "/api/auth/verify-otp" }

if (-not $verifyOtpResource) {
    Write-Host "ERROR: /api/auth/verify-otp resource not found!" -ForegroundColor Red
    exit 1
}

$resourceId = $verifyOtpResource.id
Write-Host "Found verify-otp resource: $resourceId" -ForegroundColor Green
...
aws apigateway create-deployment `
    --rest-api-id $API_ID `
    --stage-name $STAGE `
    --description "Fix CORS for verify-otp endpoint" `
    --region $REGION | Out-Null

Write-Host "`n✓ CORS configuration fixed successfully!" -ForegroundColor Green
Write-Host "`nEndpoint: https://$API_ID.execute-api.$REGION.amazonaws.com/$STAGE/api/auth/verify-otp" -ForegroundColor Cyan
Write-Host "`nTest with:" -ForegroundColor Yellow
Write-Host "curl -X OPTIONS https://$API_ID.execute-api.$REGION.amazonaws.com/$STAGE/api/auth/verify-otp -v" -ForegroundColor White
